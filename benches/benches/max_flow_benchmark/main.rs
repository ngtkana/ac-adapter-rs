...
/// Current-edge 構造を使わないと指数時間になる例 by MiSawa
...
/// Dinic の phase 数が N - 1 になる例 by Gary R. Waissi
fn max_flow_worst_gary(c: &mut Criterion) {
    let n = 3_000;
    c.bench_function(&format!("max_flow_worst_gary_n{n}"), |bencher| {
        let source = 0;
        let sink = n - 1;
        let mut edges = vec![];
        for i in 0..n - 1 {
