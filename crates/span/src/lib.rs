...
/// slice の機能を増やします。
pub trait Span<T>: ops::Index<usize> {
    /// 長さです。
    fn len(&self) -> usize;
    ...
    where
        F: FnMut(&'a Self::Output) -> cmp::Ordering,
    {
    ...
    fn span_partition_point<'a, F>(&'a self, mut pred: F) -> usize
    where
        F: FnMut(&'a Self::Output) -> bool,
    {
        let mut left = 0;
