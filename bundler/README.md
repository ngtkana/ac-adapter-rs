...
- 複数クレートを一度に指定、内部依存を自動収集してdedup（ダイヤモンド依存も1回だけ展開）
- 既存ファイルへの差分追記（`--skip-from`）
- デフォルトでdocコメント・テストを除去してコード量を削減（`--keep-docs` / `--keep-tests` で保持）
- シェル補完（zsh/bash）
- Neovim連携（`:AcBundle`）
...
acbundle fp_fps --keep-docs --keep-tests > bundled.rs   # docコメント・テストを保持
```
